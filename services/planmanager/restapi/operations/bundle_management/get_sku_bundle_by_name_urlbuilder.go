...
func (o *GetSkuBundleByNameURL) Build() (*url.URL, error) {
	...
	var _path = "/sku/bundle/name/{name}"

	name := o.Name
	if name != "" {
		_path = strings.Replace(_path, "{name}", name, -1)
	} else {
		return nil, errors.New("name is required on GetSkuBundleByNameURL")
	}
...
func (o *GetSkuBundleByNameURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetSkuBundleByNameURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetSkuBundleByNameURL")
	}
